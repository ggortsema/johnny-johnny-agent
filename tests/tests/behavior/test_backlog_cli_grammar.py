...
def test_publish_command_is_removed():
    result = runner.invoke(app, ["backlog", "publish", "--help"])

    assert result.exit_code != 0
    assert "No such command" in result.output


def test_generate_command_remains_available_for_future_migration():
    result = runner.invoke(app, ["backlog", "generate", "--help"])
...
def test_create_issue_help_uses_database_project_identity_not_yaml_file():
...
def test_update_help_uses_database_project_identity_not_yaml_file():
...
def test_move_help_uses_database_project_identity_not_yaml_file():
