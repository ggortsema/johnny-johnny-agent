...
from johnny_johnny_agent.api.app import app
...
class _AsgiTestClient:
    ...
    def post(self, path: str, *, params=None, json=None, content=None, headers=None) -> _Response:
        return self.request(
            "POST", path, params=params, json_body=json, content=content, headers=headers
        )

    def patch(self, path: str, *, params=None, json=None, content=None, headers=None) -> _Response:
        return self.request(
            "PATCH", path, params=params, json_body=json, content=content, headers=headers
        )

    def delete(self, path: str, *, params=None, json=None, content=None, headers=None) -> _Response:
        return self.request(
            "DELETE", path, params=params, json_body=json, content=content, headers=headers
        )
...
def test_liveness_and_openapi_expose_the_versioned_api():
    ...
    assert f"{PROJECT_PATH.replace('Test%20Project', '{project_title}')}/summary" in openapi["paths"]
...
def test_readiness_reports_database_schema_state(monkeypatch):
    ...
    assert response.json()["present_expected_table_count"] == 9
    assert response.json()["schema"] == "johnny_johnny"
...
def test_readiness_returns_503_when_canonical_schema_is_incomplete(monkeypatch):
    ...
    assert response.json()["missing_tables"] == ["backlog_items"]
...
def test_item_filters_reject_include_and_exclude_together(monkeypatch):
    monkeypatch.setattr(api_routes, "load_backlog_from_postgres", lambda **kwargs: _backlog())
...
def test_delete_issue_dispatches_modes(monkeypatch):
    ...
    def preview(**kwargs):
        calls.append(("preview", kwargs))
        return BacklogDeleteResult(issue=issue, parent_epic=parent, github_issue_deleted=False)

    def delete(**kwargs):
        calls.append(("delete", kwargs))
        return BacklogDeleteResult(issue=issue, parent_epic=parent, github_issue_deleted=True)
...
def test_reconciliation_dispatches_modes_and_serializes_operations(monkeypatch):
    ...
    monkeypatch.setattr(api_routes, "preview_reconcile_backlog_from_postgres", fake_preview)
...
def test_purge_dispatches_modes_and_preserves_scope(monkeypatch):
    ...
    monkeypatch.setattr(api_routes, "purge_backlog_projection_from_postgres", fake_purge)
...
def test_export_returns_portable_yaml_and_counts(monkeypatch):
    ...
    monkeypatch.setattr(api_routes, "export_backlog_yaml_text_from_postgres", fake_export)
...
def test_not_found_and_consistency_failures_map_to_stable_http_errors(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(RuntimeError("Backlog item not found: missing")),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            TargetedBacklogMutationConsistencyError("GitHub deleted; database rollback failed")
        ),
    )
